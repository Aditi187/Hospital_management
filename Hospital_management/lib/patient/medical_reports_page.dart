...
import 'package:hospital_management/services/firestore_service.dart';
...
class _MedicalReportsPageState extends State<MedicalReportsPage> {
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (data['hasPrescription'] == true &&
                              data['prescriptionTotalCost'] != null)
                            Container(
        ...
        Padding(
          ...
          child: Row(
            children: [
              Expanded(
                child: Text(
                  'Your prescriptions. You can also upload previous prescriptions (image or PDF) for your doctor to review.',
              ...
              ElevatedButton.icon(
                ...
                onPressed: () => _pickAndUploadPrescription(userId),
  ...
  Future<void> _pickAndUploadPrescription(String userId) async {
    try {
      ...
      if (picked.size > maxBytes) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('File too large. Max 10MB allowed.')),
        );
        return;
      }
      ...
      if (bytes == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Unable to read file bytes. Try again.'),
          ),
        );
        return;
      }

      // Delegate upload to FirestoreService so audit logging and storage
      // operations are centralized.
      final downloadUrl = await FirestoreService.uploadPatientPrescription(
        patientId: userId,
        bytes: bytes,
        fileName: picked.name,
        contentType: picked.extension == 'pdf' ? 'application/pdf' : null,
      );
